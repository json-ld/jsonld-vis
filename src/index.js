...
export default function (d3, jsonld, selector, config = {}) {
  ...
  let h = config.h || 600,
    ...
    transitionEase = config.transitionEase || d3.easeCubicInOut,
  ...
  function update(source) {
    ...
    const nodeSel = node.join(
      enter => {
        ...
        g.append('circle')
          .attr('r', 0)
          .style('stroke-width', d => (d.data.isIdNode ? '2px' : '1px'))
          .style('stroke', d => (d.data.isIdNode ? '#F7CA18' : '#4ECDC4'))
          .style('fill', d => {
    ...
    let maxSpan = d3.max(nodes, d => d.y + maxLabelWidth) || 0;
    ...
    // note: exit handled in join above; ensure exiting circles/text animate to zero
    // (the exit transition above removes the group; selecting inside removed groups
    //  is unnecessary here)
  ...
  function click(event, d) {
    ...
    if (d.children) {
      d.children.forEach(child => {
        if (child.data.isBlankNode && child._children) click(null, child);
      });
    }
  }
